...
Windows专用剪贴板AI翻译工具 v1.6.6，支持20+语言模型。

## ⚠️ 已知问题 (v1.6.6)

### 快捷键录制功能问题
- **问题描述**：快捷键录制功能在当前版本无法正常工作
- **原因**：Windows键盘钩子(SetWindowsHookEx)需要CGO支持，而从WSL2/Linux交叉编译时CGO默认禁用
- **影响**：无法录制自定义快捷键，特别是单独的修饰键(Ctrl/Alt/Shift)
- **临时解决方案**：使用默认快捷键或在配置文件中手动设置

### 技术限制
- 从Linux/WSL2交叉编译到Windows时，无法使用需要回调函数的Windows API
- 纯Go实现无法支持SetWindowsHookEx等需要DLL注入的功能
- 当前使用的是功能受限的stub实现
